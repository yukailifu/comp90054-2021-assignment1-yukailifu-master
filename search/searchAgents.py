...
def manhattanHeuristic_task3(position, food, info={}):
    "The Manhattan distance heuristic for a PositionSearchProblem"
    xy1 = position
    xy2 = food
    return abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
...
def foodHeuristic(state, problem):
    ...
    if (max_distance[0] == (0, 0) and max_distance[1] == (0, 0)):
        ...
        x1 = position[0]
        y1 = position[1]
        x2 = food_available[0][0]
        y2 = food_available[0][1]
        for index in range(abs(x1-x2)):
            for capsule in list_cap:
                if x1 < x2 :
                    if capsule[0] == x1 + index + 1 and (capsule[1] ==y2 or capsule[1] == y1):
                        hvalue = hvalue - 1
                        break
                if x1 > x2 :
                    if capsule[0] == x1 - index - 1  and (capsule[1] ==y2 or capsule[1] == y1):
                        hvalue = hvalue - 1
                        break
        for index in range(abs(y1-y2)):
            for capsule in list_cap:
                if y1 < y2 :
                    if capsule[1] == y1 + index + 1 and (capsule[0] ==x1 or capsule[0] == x2):
                        hvalue = hvalue - 1
                        break
                if y1 > y2 :
                    if capsule[1] == y1 - index - 1 and (capsule[0] ==x1 or capsule[0] == x2):
        ...
        x2 = max_distance[0][0]
        y2 = max_distance[0][1]
        dis_food1 = util.manhattanDistance(position,max_distance[0])
        dis_food2 = util.manhattanDistance(position,max_distance[1])
        for index in range(abs(x1 - x2)):
            for capsule in list_cap:
                if x1 < x2:
                    if capsule[0] == x1 + index + 1 and (capsule[1] ==y2 or capsule[1] == y1):
                        dis_food1 = dis_food1-1
                        break
                if x1 > x2:
                    if capsule[0] == x1 - index - 1 and (capsule[1] ==y2 or capsule[1] == y1) :
                        dis_food1 = dis_food1 - 1
        ...
        hvalue = util.manhattanDistance(position,start) + util.manhattanDistance(start,end)
        list_cap = list(capsules)
        ...
        for index in range(abs(x1 - x2)):
            for capsule in list_cap:
                if x1 < x2:
                    if capsule[0] == x1 + index + 1 :
                        hvalue = hvalue-1
                        list_cap.remove(capsule)
                        break
                if x1 > x2:
                    if capsule[0] == x1 - index - 1 :
                        hvalue = hvalue - 1
